...
def IRTest(texts, ir, num_cluster, percentage):
    ...
    for cluster in range(num_cluster):
        ...
        #ganti jadi ini
        TN = len(ir.corpus.TFIDF) - TP - FP - FN
        """
...
whole_summary = np.zeros(shape=(4))
for c in cluster:
    for p in percentage:
        print("cluster = %f, percentage=%f" % (c, p))
        summary = IRTest(data, ir, c, p)
        summary = np.sum(summary, axis=0)
        whole_summary += summary

whole_summary = np.reshape(whole_summary, (2,2))
contingency_table = pandas.DataFrame(whole_summary, index=("P Relevan", "P Tidak Relevan"), columns=("GT Relevan", "GT Tidak Relevan"))
seaborn.heatmap(contingency_table, annot=True, cbar=False)
print(whole_summary)
'''        
ir = IR("deeplearning")
